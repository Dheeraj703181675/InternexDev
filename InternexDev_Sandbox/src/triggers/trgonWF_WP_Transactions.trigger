trigger trgonWF_WP_Transactions on WF_WP_Transactions__c (before insert,before update,after insert,after Update) {
    if(Trigger.isBefore)
    {
        system.debug('<-------WF WhitePaper After Trigger---->');
        if(Trigger.isInsert)
        {
            Set<string> matchingstring_set = new Set<String>();
            for(WF_WP_Transactions__c WPT : Trigger.New)
            {
                matchingstring_set.add(WPT.Deposit_date__c + WPT.Lockbox_no__c + WPT.Batch__c + WPT.Transaction_Number__c);
            }
            ...
            for(WF_WP_Transactions__c WPT : Trigger.New)
            {
                string matchingstring = WPT.Deposit_date__c + WPT.Lockbox_no__c + WPT.Batch__c + WPT.Transaction_Number__c;
                if(MapofEnvelopeData != NULL && MapofEnvelopeData.containsKey(matchingstring))
                {
                    WPT.WF_Envelope__c = MapofEnvelopeData.get(matchingstring).id;
                }
            }
        }
    }
    if(Trigger.isAfter)
    {
        ...
        try{
            if(Trigger.isInsert ||Trigger.isUpdate)
            {
                ...
                for(WF_WP_Transactions__c WFT : Trigger.New)
                {
                        ...
                        if(mapof_PR.containsKey(WFT.Matching_string_for_Image_mapping__c))
                        {
                            for(WF_PR__c PR : mapof_PR.get(WFT.Matching_string_for_Image_mapping__c))
                            {
                                if(WFT.Front_image_file__c != NULL && mapof_LockboxImage.containsKey(WFT.Front_image_file__c.toUpperCase().trim()))
                                {
                                    ...
                                    /*      mapof_LockboxImage.get(WFT.Front_image_file__c.toUpperCase().trim()).WF_PR__c = mapof_PR.get(WFT.Matching_string_for_Image_mapping__c).id;
if(mapof_PR.get(WFT.Matching_string_for_Image_mapping__c).Payment__c != NULL)
mapof_LockboxImage.get(WFT.Front_image_file__c.toUpperCase().trim()).Payment__c = mapof_PR.get(WFT.Matching_string_for_Image_mapping__c).Payment__c; */
                                }
                                if(WFT.Rear_Image_File__c != NULL && mapof_LockboxImage.containsKey(WFT.Rear_Image_File__c.toUpperCase().trim()))
                                {
                                    ...
                                    /* mapof_LockboxImage.get(WFT.Rear_Image_File__c.toUpperCase().trim()).WF_PR__c = mapof_PR.get(WFT.Matching_string_for_Image_mapping__c).id;
if(mapof_PR.get(WFT.Matching_string_for_Image_mapping__c).Payment__c != NULL)
mapof_LockboxImage.get(WFT.Rear_Image_File__c.toUpperCase().trim()).Payment__c = mapof_PR.get(WFT.Matching_string_for_Image_mapping__c).Payment__c; */
                                }
                            }
                        }
                    }
